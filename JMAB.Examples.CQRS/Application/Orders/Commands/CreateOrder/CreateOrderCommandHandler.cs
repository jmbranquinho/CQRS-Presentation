...
public class CreateOrderCommandHandler : ICommandHandler<CreateOrderCommand, Guid>
{
    ...
    public CreateOrderCommandHandler(IOrderRepository repository, IEmailService emailService)
    {
    ...
    public async Task<Guid> HandleAsync(CreateOrderCommand command, CancellationToken cancellationToken = default)
    {
        ...
        await _emailService.SendOrderCreatedEmailAsync(order.Id, order.Name, cancellationToken);

        // mediator.Publish(new OrderCreatedEvent(order.Id, order.Name), cancellationToken);
